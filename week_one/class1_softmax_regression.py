...
def h_gradient(theta, X, y, lam=0.1):
    ...
    grad = (-1 / n) * np.dot(X.T, (y_mat - preds)) + lam * theta
    return grad


def softmax_cost_func(theta, X, y, lam=0.1):
    n = X.shape[0]  # Traning samples #
    y_mat = onehot(y)

    # TODO compute loss (cost)
    preds = h_func(theta, X)
    loss = (-1 / n) * np.sum(y_mat * np.log(preds)) + (lam / 2) * np.sum(theta * theta)
    return loss
...
def softmax_regression():
    ...
    fitted_val, cost_iter = softmax_grad_desc(theta, X, y, lr=0.0001, max_iter=1000, lam=1)
