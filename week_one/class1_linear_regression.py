...
def linear_grad_func(theta, x, y):
    ...
    h_hat = linear_val_func(theta, x)
    error = (h_hat - y)
    grad = np.dot(np.c_[np.ones(x.shape[0]), x].T, error) / x.shape[0]
    return grad
...
def linear_cost_func(theta, x, y):
    ...
    cost = np.sum((linear_val_func(theta, x) - y) ** 2) / (x.shape[0] * 2)
    return cost
